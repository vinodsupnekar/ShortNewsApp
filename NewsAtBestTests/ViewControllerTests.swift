...
final class ViewControllerTests: XCTestCase {
    ...
    func assertThat(_ sut: ViewController, hasViewConfiguredFor news: NewsFeed, at index: Int, file: StaticString = #file, line: UInt = #line) {
        ...
        XCTAssertEqual(cell.title.text, news.title, "Expected location text to be \(String(describing: news.title)) for image  view at index (\(index))", file: file, line: line)

        XCTAssertEqual(cell.descriptionLabel.text, news.description, "Expected description text to be \(String(describing: news.description)) for image view at index (\(index)", file: file, line: line)
    }
}
